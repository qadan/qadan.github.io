...
conn = psycopg2.connect(dbname='garnish_db', user='burgerboy', password='burgers2017', host='meatadata.info')
cur = conn.cursor()
main = urllib2.urlopen('http://www.peiburgerlove.ca').read()
...
for restaurant in restaurants:
  ...
  burgerinfo = {
      ...
      'quote': burgerpage.find('blockquote', id="Burger-Quote-Container").text[2:-2].strip(),
      'ingredients': burgerpage.find('div', id="Ingredients-Container").string.strip(),
      'url_suffix': url_suffix,
  }

  cur.execute('insert into burgers values (%s, %s, %s, %s, %s)', [None, burgerinfo['name'], burgerinfo['quote'], burgerinfo['ingredients'], burgerinfo['url_suffix']])
  ...
  for restaurant_div in restaurant_divs:
    ...
    restaurantinfo = {
      'name': burgerpage.find(div, id='Restaurant-Name').text,
    ...
    cur.execute('insert into restaurants values (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)', [None, restaurantinfo['name'], restaurantinfo['phone_number'], restaurantinfo['address'], restaurantinfo['hours_of_operation'], None, restaurantinfo['latitude'], restaurantinfo['longitude'], restaurantinfo['website'], restaurantinfo['site_id']])
    restaurant_ids.append(cur.lastrowid)

  for restaurant_id in restaurant_ids:
    cur.execute('insert into restaurant_burgers values (%s, %s, %s)', [None, burger_id, restaurant_id])
